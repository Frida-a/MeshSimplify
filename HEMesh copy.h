...
public:
    ...
    void preCalcu();
    void EdgeCollaspe(EdgeIterator target_it , Point NewPt);
...
template<typename DType>
void Mesh<DType>::EdgeCollaspe(EdgeIterator target_it , Point NewPt){
    ...
    oldFaceIts.push_back(edge_it->m_pair_it->m_face_it);
...
void Mesh<DType>::meshSimplfy(int target_face_num){
    ...
    //make_heap(m_edge_cost.begin(),m_edge_cost.end(),greater<Edge_cost>());
    //do edge collapse
    ...
    while(face_num > target_face_num){
        ...
        if(tmpEc_before.m_edge_half == tmpEc.m_edge_half->m_pair_it){  continue;}//skip pair edge
        ...
        std::cout<<"edge num    "<<tmpEdgeIts.size()<<"vert num"<<tmpVertIts.size()<<std::endl;
        /*
        for(int i = 0  ; i<tmpEdgeIts.size() ; i++){
            if( tmpEdgeIts[i]->m_next_it->m_next_it != target_it 
            || tmpEdgeIts[i]->m_pair_it->m_next_it != target_it->m_pair_it){
                flag = 1;
        ...
        */
        if(tmpEdgeIts.size()>2){ flag = 1;}

        if(flag == 0){
            EdgeCollaspe(target_it,target_it->best_pt);
